...
gine_ssl_no_d_sb_auc = GINE_SSL_no_D_SB_data['AUC'].values.tolist()
gine_ssl_d_sb_auc = GINE_SSL_D_SB_data['AUC'].values.tolist()

auc_all = [np.mean(gp_auc), np.mean(gp_d_auc), np.mean(rf_auc), np.mean(rf_d_auc), np.mean(dt_auc), np.mean(dt_d_auc), np.mean(svm_auc), np.mean(svm_d_auc), 
...
type_all_d_sb = ['D-, SB-', 'D+, SB-', 'D-, SB+', 'D+, SB+'] * 2
error_all_d_sb = [
    np.std(gcn_ssl_auc), np.std(gcn_ssl_d_auc), np.std(gcn_ssl_no_d_sb_auc), np.std(gcn_ssl_d_sb_auc),
    np.std(gine_ssl_auc), np.std(gine_ssl_d_auc), np.std(gine_ssl_no_d_sb_auc), np.std(gine_ssl_d_sb_auc)
]
...
class_order = ['D-, SB-', 'D+, SB-', 'D-, SB+', 'D+, SB+']
