...
cids = []
for cid in cid_list:
    if cid in label_cid_list:
        label = int(label_df.loc[label_df['cid'] == cid, 'label'].values[0])
        if label != 1:
            continue
...
all_labels_kmeans = kmeans.fit_predict(X_scaled)
labels_kmeans = all_labels_kmeans[:pred_len]
real_labels_kmeans = all_labels_kmeans[pred_len:]
...
labeled_df['cid'] = cids[pred_len:]
labeled_df['formula'] = formulas[pred_len:]
labeled_df['smiles'] = smiles[pred_len:]
labeled_df['x'] = labeled_emb_3d[:, 0]
labeled_df['y'] = labeled_emb_3d[:, 1]
labeled_df['z'] = labeled_emb_3d[:, 2]
labeled_df['cluster'] = [i + 1 for i in real_labels_kmeans.tolist()]
...
for cluster_id in range(n_clusters):
    idx = real_labels_kmeans == cluster_id
    # plt.scatter(labeled_emb_2d[idx, 0], labeled_emb_2d[idx, 1],
    #             color=colors[cluster_id],
    #             marker='X',
    #             label=f'Real label in cluster {cluster_id + 1}',
    #             edgecolors='black',
    #             s=45)
    ax.scatter(labeled_emb_3d[idx, 0], labeled_emb_3d[idx, 1], labeled_emb_3d[idx, 2],
                color=colors[cluster_id],
                marker='X',
                label=f'Real label in cluster {cluster_id + 1}',
                edgecolors='black',
...
plt.tight_layout()
plt.grid(True)
plt.savefig('./data/kmeans_clusters_3d.png', dpi=600)
